...
/**
 * Course advanced completion progress report
 *
 * @package    report
...
require_once("{$CFG->libdir}/gradelib.php"); //tk
require_once("{$CFG->dirroot}/grade/querylib.php"); //tk
...
$url = new moodle_url('/report/adv_completion/index.php', array('course'=>$course->id));
$PAGE->set_url($url);
...
require_capability('report/adv_completion:view', $context);
...
if (!$completion->has_criteria()) {
    print_error('nocriteriaset', 'adv_completion', $CFG->wwwroot.'/course/report.php?id='.$course->id);
}
...
if ($csv) {
    ...
    $export->set_filename('adv_completion-'.$shortname);
    ...
    groups_print_course_menu($course, $CFG->wwwroot.'/report/adv_completion/index.php?course='.$course->id);
}
...
if (!$grandtotal) {
    echo $OUTPUT->container(get_string('err_nousers', 'adv_completion'), 'errorbox errorboxcontent');
    echo $OUTPUT->footer();
    exit;
}
...
$link = $CFG->wwwroot.'/report/adv_completion/index.php?course='.$course->id;
if (strlen($sort)) {
    $link .= '&amp;sort='.$sort;
}
$link .= '&amp;start=';
...
if (!$csv) {
	...
	//tk grade header
	echo '<th scope="col" class="completion-identifyfield">' .
            get_string('grade', 'report_adv_completion') . '</th>';
    ...
    foreach ($criteria as $criterion) {
        ...
        if ($criterion->criteriatype == COMPLETION_CRITERIA_TYPE_ACTIVITY) {
            ...
            $row[] = $formattedname . ' - ' . get_string('completiondate', 'report_adv_completion');
        }
        else {
...
//tk get all grades
$grading_info = grade_get_course_grades($course->id, array_keys($progress));
...
foreach ($progress as $user) {
	...
	//tk round user's course grade
	$grade = grade_format_gradevalue($grading_info->grades[$user->id]->grade, $grade_item);
    ...
    foreach ($criteria as $criterion) {
        ...
        if ($csv) {
            ...
            if ($allow_marking_criteria === $criterion->id) {
				...
				//$backto = $url; //tk
				//$backto->set_anchor('user-'.$user->id); //tk

                $toggleurl = new moodle_url(
                    //'/course/togglecompletion.php',
                    '/report/adv_completion/togglecompletion.php', //tk
                    array(
                        'user' => $user->id,
                        'course' => $course->id,
						...
						'grader' => $USER->id,
						'grade' => $grade,
						'gradeitem' => $grade_item->id//,
						//'backto' => $backto->out() //tk
                    )
                );

                print '<a href="'.$toggleurl->out().'" title="'.s(get_string('clicktomarkusercomplete', 'report_adv_completion')).'">' .
...
$csvurl = new moodle_url('/report/adv_completion/index.php', array('course' => $course->id, 'format' => 'csv'));
$excelurl = new moodle_url('/report/adv_completion/index.php', array('course' => $course->id, 'format' => 'excelcsv'));
...
$event = \report_adv_completion\event\report_viewed::create(array('context' => $context));
$event->trigger();
