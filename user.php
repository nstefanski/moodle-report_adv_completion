...
/**
 * Display user advanced completion report
 *
 * @package    report
...
require('../../config.php');
require_once($CFG->dirroot.'/report/adv_completion/lib.php');
...
if (!report_adv_completion_can_access_user_report($user, $course)) {
    ...
    print_error('nocapability', 'report_adv_completion');
}

$stractivityreport = get_string('activityreport');

$PAGE->set_pagelayout('admin');
$PAGE->set_url('/report/adv_completion/user.php', array('id'=>$user->id, 'course'=>$course->id));
...
if (!$rs->valid()) {
    ...
    if ($course->id != 1) {
        $error = get_string('nocompletions', 'report_adv_completion'); // TODO: missing string
    } else {
        $error = get_string('nocompletioncoursesenroled', 'report_adv_completion'); // TODO: missing string
    }
...
foreach ($courses as $type => $infos) {
    ...
    if (!empty($infos)) {
        ...
        echo '<h1 align="center">'.get_string($type, 'report_adv_completion').'</h1>';
        ...
        if ($type === 'complete') {
            echo '<th class="c4 header" scope="col">'.get_string('completiondate', 'report_adv_completion').'</th>';
        }
        ...
        foreach ($infos as $c_info) {
            ...
            if (!empty($activities)) {
                ...
                $row = array();
                $row['title'] = get_string('activitiescomplete', 'report_adv_completion');
            ...
            foreach ($rows as $row) {
                ...
                switch ($row['status']) {
                    ...
                    case 'No':
                        echo get_string('incomplete', 'report_adv_completion');
                        break;
                ...
                echo '</td><td class="c3">';
                if ($first_row) {
                    echo '<a href="'.$CFG->wwwroot.'/blocks/completionstatus/details.php?course='.$c_course->id.'&user='.$user->id.'">'.get_string('detailedview', 'report_adv_completion').'</a>';
                }
                echo '</td>';
...
$event = \report_adv_completion\event\user_report_viewed::create(array('context' => $coursecontext, 'relateduserid' => $userid));
$event->trigger();
